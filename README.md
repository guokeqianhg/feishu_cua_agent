...
我们已将项目核心功能录制成完整 Demo 视频，并统一放在展示链接中。视频覆盖飞书 IM、云文档、日历、视频会议四个子产品的真实桌面端操作流程，包括自然语言任务解析、桌面观察、控件定位、鼠标键盘执行、状态验证和运行报告生成。
Demo 的重点不是展示单个脚本能否点击成功，而是展示一个 Computer-Use Agent 如何在真实飞书桌面环境中完成端到端任务闭环。Agent 不调用飞书开放平台 API，而是通过截图、OCR、视觉模型、窗口控制和桌面自动化，模拟真实用户完成操作。因此，Demo 能体现项目在真实办公软件场景中的落地能力，也能体现系统对复杂 UI 状态、弹窗、窗口焦点和误操作风险的处理能力。
Demo 视频链接：
https://space.bilibili.com/694526903
