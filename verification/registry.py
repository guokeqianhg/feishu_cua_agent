...
def local_verify_step(
    ...
    if verifier == "docs_shared":
        ...
        invite_pending = _docs_share_invite_pending(after.screenshot_path)
        ok = _healthy_image(after.screenshot_path) and (not search_open) and (not sending) and (not invite_pending)
        return (
            _pass(f"Docs share action completed and recipient search is no longer open. image_diff={diff:.2f}.", verifier, 0.82)
            if ok
            else _fail(
                (
                    "Docs share completion was not confirmed. "
                    f"image_diff={diff:.2f}, recipient_search_open={search_open}, "
                    f"sending={sending}, invite_pending={invite_pending}."
                ),
                "verification_failed",
                verifier,
            )
...
def local_verify_case(case: TestCase, final_observation: Observation | None) -> StepVerification | None:
    ...
    if template in {
        ...
        if template in {"calendar_invite_attendee_guarded", "calendar_modify_event_time_guarded"} and not event_time:
            event_time = "明天 10:00"
        if template == "calendar_modify_event_time_guarded":
            event_time = str(case.metadata.get("new_event_time") or event_time)
        event_date, start_time, _end_time = _calendar_time_parts(event_time) if event_time else ("", "", "")
        step = PlanStep(
    ...
    if template in {"vc_start_meeting_guarded", "vc_join_meeting_guarded", "vc_toggle_devices_guarded"}:
        ...
        if template == "vc_toggle_devices_guarded":
            return _verify_vc_state(None, final_observation, step, "vc_in_meeting")
    ...
    if template == "docs_share_doc_guarded":
        ...
        invite_pending = _docs_share_invite_pending(final_observation.screenshot_path)
        if _healthy_image(final_observation.screenshot_path) and (not search_open) and (not sending) and (not invite_pending):
            return _pass("Docs share flow completed and final share dialog is stable.", "docs_share_case", 0.86)
        return _fail(
            f"Docs share final state was not stable: recipient_search_open={search_open}, sending={sending}, invite_pending={invite_pending}.",
            "verification_failed",
            "docs_share_case",
        )
...
def _vc_start_dialog_visible(window_title: str | None, normalized_text: str) -> bool:
    title = _normalize_text(window_title or "")
    if not any(item in title for item in ("飞书会议", "feishumeeting", "larkmeeting")):
        return False
    if any(item in normalized_text for item in ("离开会议", "结束会议", "participants", "leave")):
        return False
    markers = (
        "的视频会议",
        "发起会议",
    ...
    return any(item in normalized_text for item in markers)
...
def _verify_vc_state(
    ...
    if verifier == "vc_join_dialog_visible":
        ...
        input_bbox = vc_strategy_bbox_from_screenshot(after.screenshot_path, "vc_meeting_id_input")
        if has_input_prompt and (input_bbox is not None or not on_home_card):
    ...
    if verifier == "vc_prejoin_or_in_meeting":
        text = _normalize_text(" ".join(after.ocr_lines or []) or " ".join(item[1] for item in ocr_image(after.screenshot_path)))
        start_dialog_visible = _vc_start_dialog_visible(after.window_title, text)
        if analysis.prejoin_visible or analysis.in_meeting_visible or start_dialog_visible:
            return _pass("VC start action reached prejoin or in-meeting state.", verifier, 0.82)
        return _fail(
            f"VC start action did not reach prejoin/in-meeting state. wrong_state={analysis.wrong_state}, start_dialog_visible={start_dialog_visible}.",
            "verification_failed",
            verifier,
        )
    ...
    if verifier == "vc_meeting_id_entered":
        ...
        text = _normalize_text(" ".join(after.ocr_lines or []))
        if meeting_id and meeting_id not in text:
            text = _normalize_text(f"{text} {' '.join(item[1] for item in ocr_image(after.screenshot_path))}")
        bbox = vc_strategy_bbox_from_screenshot(after.screenshot_path, str(step.metadata.get("locator_strategy") or "vc_meeting_id_input"))
        ...
        join_action_visible = any(item in text for item in ("加入会议", "joinmeeting", "join"))
        hit = bool(meeting_id and meeting_id in text and (has_input_prompt or join_action_visible))
        ...
        if hit:
            return _pass(
                f"VC meeting ID entry was locally confirmed by dialog OCR. text_hit={hit}, roi_hit={roi_hit}, crop_diff={changed:.2f}.",
                verifier,
                0.82,
            )
        if changed > 0.8 and bbox is not None:
            return _pass(
                f"VC meeting ID input ROI changed after typing. text_hit={hit}, roi_hit={roi_hit}, crop_diff={changed:.2f}.",
                verifier,
                0.78,
            )
        return _fail(
    ...
    if verifier == "vc_meeting_title_entered":
        ...
        bbox = vc_strategy_bbox_from_screenshot(after.screenshot_path, str(step.metadata.get("locator_strategy") or "vc_meeting_title_input"))
        changed = _crop_diff(before.screenshot_path if before else None, after.screenshot_path, bbox.model_dump()) if bbox else 0.0
        roi_text = _normalize_text(" ".join(item[1] for item in ocr_image(after.screenshot_path, bbox))) if bbox else ""
        ...
        if changed > 0.8 and bbox is not None:
            return _pass(
                f"VC meeting title input ROI changed after typing. roi_text={roi_text!r}, crop_diff={changed:.2f}.",
                verifier,
                0.78,
            )
        return _fail(
...
def _verify_calendar_time_axis_target_visible(after: Observation, step: PlanStep, verifier: str) -> StepVerification:
    ...
    event_title = str(step.metadata.get("event_title") or "").strip()
    if event_title:
        event_visible = _verify_calendar_event_visible(after, step, verifier)
        if event_visible.success:
            return _pass(
                f"Calendar saved event is already visible after scrolling. {event_visible.reason}",
                verifier,
                max(0.84, event_visible.confidence),
            )
...
def _docs_share_invite_pending(path: str) -> bool:
    if not Path(path).exists():
        return False
    normalized = _normalize_text(_docs_share_modal_ocr_text(path))
    has_invite_form = any(item in normalized for item in ("邀请协作者", "发送通知", "添加备注", "可阅读"))
    has_send_action = "发送" in normalized or "send" in normalized or "invite" in normalized
    return has_invite_form and has_send_action
